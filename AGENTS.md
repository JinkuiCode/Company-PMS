...
- 项目进度功能结构未最终定稿前，不要大改信息架构；先保证风格和基础交互一致。这里的大改包括重新组织导航层级、模块边界、主要操作路径或核心字段归属；结构定稿或结构变更仍须用户明确确认。本条不要求为已批准范围内的样式修复和基础交互修复逐项重新确认。
...
- 涉及 OA、ERP、认证、菜单权限、数据权限的改动，应在该批实施方案中明确改动对象、接口影响和回滚风险，待用户确认后实施。已批准的同一范围不逐文件重复确认；出现未覆盖的接口或权限影响时，只暂停受影响部分并说明差异。以下权限约束继续适用。
- 后端业务接口必须通过统一 RBAC 依赖校验按钮权限，并在读写项目、任务、档案和 ERP 数据时同时校验数据范围与产品类别范围。
...
- 产品类别、设备系列等可配置枚举允许维护值；档案、项目和任务状态属于固定流程枚举。业务枚举统一使用系统分配的数字流水存储值，存储值创建后不可修改且删除后不得复用；只允许维护显示名、排序和启停。
...
- 生产启动只检查数据库版本，不执行建表或迁移；结构升级必须在已验证备份和单独批准的维护窗口内，通过 `backend/scripts/upgrade_database.py` 执行。
- 任何模型结构、数据库迁移或初始化数据逻辑变更，必须同步提升 `backend/app/services/database_revision.py` 中的版本并补充升级契约测试。不得通过伪造版本标记跳过升级。
- 数据库升级身份与日常运行身份分离；升级凭据不得写入代码、命令参数、日志或 Git。权限收紧与生产部署仍需单独批准和回退准备。

## 验证命令

前端改动时，将下列命令纳入该批实施方案的验证项。已批准且仍适用的验证方案可以复用；已有验证证据仅在相关代码、输入和环境未变化时复用。不得把预期的 TDD RED 失败当成整个任务的阻塞，也不得把未通过的必需验收项报告为完成。
...
- 确需新建功能分支、且用户没有指定起点和分支名称时，默认从 `master` 创建 `codex/` 前缀分支。用户已指定可用分支或 worktree 时直接复用，不为进入下一阶段再次新建或移动工作区。创建 worktree 和直接在主分支实施仍遵守各自明确同意要求。
- 合并到 `master` 属于正式程序更新，必须获得针对该合并动作的明确确认；一般实施方案批准不自动包含合并许可。同一合并动作已明确获准时，不重复询问。
- 合并后可删除已合并的临时功能分支。删除前核实目标分支已包含其提交，且该分支不承载未合并工作、未被其他任务使用；这项许可不包括强制删除未合并分支或丢弃工作区改动。
