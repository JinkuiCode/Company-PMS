...
- 标准主从联动：详情已打开时，点击列表其他记录必须同步切换详情，不要求再次点编辑。仅点击行不得自动打开原本关闭的详情，也不得直接进入字段编辑。
- 主从切换复用 `createDetailSwitch`；有草稿时使用共享 `confirmDetailSwitch` 提供保存后切换、放弃修改并切换、取消。保存失败不得切换或丢弃草稿；快速点击只应用最后一次选择，旧异步响应不得覆盖新记录，加载/保存期间禁止编辑错配数据。新增页面必须补充相应行为测试。
...
- `business:data:all` 是明确可撤销的“全部业务数据”授权，覆盖部门、产品线及未归属数据范围，仅首次升级默认授予系统管理员。它不自动赋予查看页面、编辑、删除、同步或导出权限，也不绕过业务校验。报表及后台导出须使用同一数据范围；空授权仍默认拒绝，禁止以缺少参数代表全部授权。
...
- 当前 PMS 尚未正式上线（2026-09-18 用户确认），已批准开发任务可随时部署 PMS，必要时重启 PMS 自身；禁止重启服务器或与 OA 共用的 SQL Server 服务。数据库结构变更仍需备份校验及独立升级流程，前端发布不得附带数据库升级。GitHub 推送、主分支合并仍按各自批准边界执行。
