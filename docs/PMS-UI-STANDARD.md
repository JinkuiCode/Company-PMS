...
- 列设置复用 `PmsListColumnPicker` 和 `PmsGridLayoutEditor`，字段显隐与布局分开。已显示字段可选择左/中/右区域、拖动或上下排序、输入列宽；表头拖动列宽与面板配置使用同一份 AG Grid 列状态，并按账号及浏览器保存。普通列受各列最小/最大宽度约束，默认 60-800px；勾选列和操作列固定，不参与自由排序及宽度调整。恢复默认同时重置显隐、冻结、顺序和宽度，不改变业务筛选条件。
