...
assert.match(view, /diff_items/, '详情应优先消费后端生成的可读差异项')
assert.match(view, /field_label/, '差异项应以中文字段名作为主要标识')
assert.match(view, /field_key/, '差异项应保留原始字段编码供技术排查')
assert.match(view, /diff-value/, '变更前后值应使用可换行的业务值样式')
